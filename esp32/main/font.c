...
/*
 * 8x16 bitmap font — VGA-style glyphs for ASCII 32 (space) through 126 (~).
 * Each glyph is 16 rows of 1 byte each. Bit 7 = leftmost pixel.
 * Based on the classic IBM VGA 8x16 font (public domain).
 *
 * Copied from src/font.c (Part 1) with UINT8 → uint8_t.
 */
const uint8_t font_data[FONT_CHARS][FONT_HEIGHT] = {
    /* 32: ' ' (space) */
