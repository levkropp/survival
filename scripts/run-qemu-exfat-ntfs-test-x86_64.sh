...
# exFAT/NTFS copy test (x86_64): boots workstation with three USB drives
...
#
# Usage: ./scripts/run-qemu-exfat-ntfs-test-x86_64.sh [path-to-iso]
...
# Temp file paths
VARS="/tmp/survival_ent_x64_vars.fd"
USB1="/tmp/survival_ntfs_src_x64.img"
USB2="/tmp/survival_exfat_dst1_x64.img"
USB3="/tmp/survival_exfat_dst2_x64.img"
MNT="/tmp/survival_mnt_$$"

# Clean up any root-owned leftovers from previous runs
for f in "$VARS" "$USB1" "$USB2" "$USB3"; do
    if [ -f "$f" ] && [ ! -w "$f" ]; then
...
echo ""
echo "=== exFAT/NTFS Copy Test - QEMU x86_64 ==="
