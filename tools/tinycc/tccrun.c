...
#ifdef __UEFI__
/* UEFI: allocate code memory below 2GB so RIP-relative addressing works */
extern void *mem_alloc_code(unsigned long size);
extern void mem_free_code(void *ptr, unsigned long size);
#endif
...
static int rt_mem(TCCState *s1, int size)
{
    ...
    int ptr_diff = 0;
#ifdef __UEFI__
    ptr = mem_alloc_code(size += PAGESIZE);
    if (!ptr)
	return tcc_error_noabort("tccrun: could not allocate code memory");
